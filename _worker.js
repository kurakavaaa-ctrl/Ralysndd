...
/*
 * Project Ralysnd - IoT Device Telemetry Gateway
...
const SYSTEM_DEFAULTS = {
    ...
    isPaused: false,
    silentAlerts: false,
    githubRepo: "kurakavaaa-ctrl/Ralysndd",
...
function generateApiKey(name) {
    ...
    const raw = `ralysnd_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
    const key = raw;
    return {
        id,
        ...
        try {
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    const dashboardUrl = env.DASHBOARD_URL || 'https://raw.githubusercontent.com/kurakavaaa-ctrl/Ralysndd/main/dashboard.html';
                    try {
                ...
                if (reqPath === routes.data) {
                    ...
                    if (isRealBrowser && !isCustomUaAllowed) {
                        if (isValidUser) {
                            const subscriptionUrl = env.SUBSCRIPTION_URL || 'https://raw.githubusercontent.com/kurakavaaa-ctrl/Ralysndd/main/subscription.html';
                            try {
    ...
    async scheduled(event, env, ctx) {
        try {
            ...
            if (sysConfig.autoUpdate && sysConfig.cfAccountId && sysConfig.cfApiToken && sysConfig.cfWorkerName) {
                const repo = (sysConfig.githubRepo || "kurakavaaa-ctrl/Ralysndd")
...
function obfuscateCode(srcText) {
    ...
    cleanCode = cleanCode.replace(/export\s+default\s+/g, "const _0xRalysndModule = ");
    cleanCode += "\nreturn _0xRalysndModule;";
    ...
    const finalLoaderCode =
        rawImportsStr +
        "\n\n" +
        "// Ralysnd Gateway - Obfuscated Loader Context (v2.5.4.2 Optimized)\n" +
        'const _0xRalysndPayload = "' +
        hexOutput +
        '";\n' +
        "const _0xRalysndKey = " +
        randKey +
        ";\n\n" +
        "const _0xRalysndBytes = new Uint8Array((_0xRalysndPayload.match(/.{1,2}/g) || []).map(x => parseInt(x, 16) ^ _0xRalysndKey));\n" +
        "const _0xRalysndCode = new TextDecoder().decode(_0xRalysndBytes);\n" +
        "const _0xRalysndRuntime = new Function(" +
        bindingNames.map((name) => '"' + name + '"').join(", ") +
        ", _0xRalysndCode)(" +
        bindingNames.join(", ") +
        ");\n\n" +
        "export default _0xRalysndRuntime;";
